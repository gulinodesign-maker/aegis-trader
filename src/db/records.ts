...
export async function recordAudit(input: {
  ...
  try {
    ...
    await sql`
      ...
      values (${input.action}, ${"system"}, ${input.entityType}, ${input.entityId ?? null}, ${sql.json(toDbJson(input.data ?? {}))})
    `;
  } catch (error) {
...
export async function recordRiskEvent(input: {
  ...
  try {
    ...
    await sql`
      ...
      values (
        ${input.proposalId ?? null}, ${input.approved}, ${sql.json(toDbJson(input.reasons))}, ${sql.json(toDbJson({}))},
        ${sql.json(toDbJson(input.checks))}, ${Number(input.checks.quantity ?? 0)}, ${Number(input.checks.maxLoss ?? 0)}
      )
    `;
  } catch (error) {
